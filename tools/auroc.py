...
from tools.plotting_utils import plot_figure
...
class AUROC(ROC):
    ...
    def generate_figure(self, stats=None) -> Tuple[Figure, str]:
        ...
        # np.savez('auroc_res.npz', x_vals=fpr.cpu(), y_vals=tpr.cpu(), auc=auroc.cpu())
        stats['x_vals'] = fpr.cpu()
        stats['y_vals'] = tpr.cpu()
        stats['auc'] = auroc.cpu()
