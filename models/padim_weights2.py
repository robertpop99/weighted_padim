...
DIMS = {
    ...
    # "alexnet": {"orig_dims": 1376, "reduced_dims": 400, "emb_scale": 4},
    "alexnet": {"orig_dims": 1024, "reduced_dims": 300, "emb_scale": 4},
    # "alexnet": {"orig_dims": 736, "reduced_dims": 400, "emb_scale": 4},
}
...
class WeightsWrapper(nn.Module):
    def __init__(self):
        super().__init__()
        self.weights = torch.ones(DIMS['resnet18']['reduced_dims'])

    def setup_weights(self, backbone, idx):
        self.weights = torch.ones(DIMS[backbone]['reduced_dims']).to(self.weights.device)
        if 'alexnet' in backbone:
            layer1 = (idx < 384).sum()
            layer2 = (idx < 768).sum()
            self.weights[:layer1] = 1
            self.weights[layer1:layer2] = 1
            self.weights[layer2:] = 1
            print(layer1, layer2 - layer1, DIMS[backbone]["reduced_dims"] - layer2)
        elif 'resnet18' in backbone:
            layer1 = (idx < 64).sum()
            layer2 = (idx < 192).sum()
            self.weights[:layer1] = 0
            self.weights[layer1:layer2] = 1
            self.weights[layer2:] = 5
            print(layer1, layer2 - layer1, DIMS[backbone]["reduced_dims"] - layer2)
        elif 'wide_resnet50_2' in backbone:
            layer1 = (idx < 256).sum()
            layer2 = (idx < 768).sum()
            self.weights[:layer1] = 0
            self.weights[layer1:layer2] = 5
            self.weights[layer2:] = 10
            print(layer1, layer2 - layer1, DIMS[backbone]["reduced_dims"] - layer2)
...
weights_wrapper = WeightsWrapper()
...
class AlexNetFeatureExtractor(nn.Module):
    def __init__(self, layers: List[str]):
        super().__init__()
        # onnx_model = onnx.load("supervised_learning/model2.onnx")
        # self.model = onnx_to_keras(onnx_model, ['data'], verbose=False)
        # self.idx = [5, 10, 14, 17, 20]
        #
        # self.output_layers = []
        # for i in self.idx:
        #     self.output_layers.append(self.model.layers[i].output)
        #
        # self.feature_extractor = K.function([self.model.layers[0].input], self.output_layers)
        with tf.io.gfile.GFile('supervised_learning/model2.pd', 'rb') as f:
        ...
        self.layers = layers
        self.output_layers = []
        # self.layer_names = ["convolution_3:0", "concat_3:0", "concat_4:0"]
        self.layer_names = ["relu3:0", "relu4:0", "relu5:0"]
        for layer in self.layer_names:
            self.output_layers.append(self.sess.graph.get_tensor_by_name(layer))

        self.input = self.sess.graph.get_tensor_by_name('data:0')
    ...
    def forward(self, input_tensor: Tensor) -> Dict[str, Tensor]:
        ...
        for i in range(input_tensor.shape[0]):
            x = input_tensor[i].view(1, *input_tensor[i].shape)
            out = self.sess.run(self.output_layers, feed_dict={self.input: x.cpu()})
            for j in range(len(self.layer_names)):
                features[j] = torch.concat((features[j], torch.tensor(out[j])))

        for i in range(len(features)):
            batch, height, width, channel = features[i].size()
            if height == width:
                # channel should be second
                features[i] = features[i].view(batch, channel, height, width)
            features[i] = features[i].to(input_tensor.device)

        rez = dict(zip(self.layers, features))
        return rez
    ...
    def compute_distance(self, embedding: Tensor, stats: List[Tensor]) -> Tensor:
        ...
        if self.distance_type == 'mahalanobis':
            ...
            for i in range(height * width):
                dist = matching_likelihood[i].to(embedding.device)
...
class MultiVariateGaussian(nn.Module):
    ...
    def forward(self, embedding: Tensor) -> List[Tensor]:
        ...
        for i in range(height * width):
            ...
            self.matching_likelihood[i] = CustomMultivariateNormal(
                self.mean[:, i].to(device), self.covariance[:, :, i].to(device))
...
class PadimModel(nn.Module):
    def __init__(self, image_size, layers, backbone: str = "resnet18", pre_trained: bool = True,
        ...
        self.layers = layers
        if 'alexnet' in backbone:
            self.feature_extractor = AlexNetFeatureExtractor(layers)
        else:
        ...
        weights_wrapper.setup_weights(backbone, self.idx)
...
class PadimWeights2Trainer(BasicTrainer):
    def __init__(self, args, train_loader, test_loader, logger=None):
        ...
        elif 'alexnet' in self.model_type:
            backbone = 'alexnet'
        else:
            backbone = "resnet18"
        pre_trained = True
        ...
        weights_wrapper.to(self.device)
    ...
    def train_step(self, batch_idx, data, labels, path) -> torch.Tensor:
        if 'alexnet' in self.model_type:
            data = F.interpolate(data, (227, 227))
    ...
    def prepare_test(self):
        if self.prepared_for_test is True:
            return

        self.prepared_for_test = True
        embeddings = torch.vstack(self.embeddings)

        # self.stats = self.model.gaussian.fit(embeddings.to(self.device))
        # self.model.gaussian.fit(embeddings.to(self.device))
        self.model.gaussian.fit(embeddings)
